...
func (e *EnvConfig) gatherInfo(prefix string, spec interface{}) ([]varInfo, error) {
	...
	for i := 0; i < s.NumField(); i++ {
		...
		if b, err := strconv.ParseBool(ftype.Tag.Get("split_words")); err != nil && e.DefaultSplitWords || err == nil && b {
		...
		if f.Kind() == reflect.Struct {
			...
			if decoderFrom(f) == nil && setterFrom(f) == nil && textUnmarshaler(f) == nil && binaryUnmarshaler(f) == nil {
				...
				embeddedInfos, err := e.gatherInfo(innerPrefix, embeddedPtr)
				if err != nil {
					return nil, err
				}
...
// a non-empty prefix.
func (e *EnvConfig) CheckDisallowed() error {
	infos, err := e.gatherInfo(e.Prefix, e.Spec)
	if err != nil {
		return err
	}
...
func (e *EnvConfig) Process() error {
	infos, err := e.gatherInfo(e.Prefix, e.Spec)
